...
identify_outliers <- function(column) {
  ...
  return(list(outliers =outliers,count= sum(outliers)))
}

select_cols <- c("PTJE_LENGUAJE", "PTJE_MATEMATICAS",
                 "PTJE_NEM", "EDAD_EGRESO_COLEGIO", "EDAD_INGRESO_UNIV")
outliers_counts <-lapply(data[, select_cols, drop = FALSE],
                         function(column) identify_outliers(column))

# Crear un gráfico de boxplot para cada variable numérica
par(mfrow=c(3,3), mar=c(4, 4, 2,1))  

for (i in seq_along(data[, select_cols, drop = FALSE])) {
  if (is.numeric(data[, select_cols, drop = FALSE][[i]])) {
    boxplot(data[, select_cols, drop = FALSE][[i]], main = paste("Boxplot de",
                                                                 names(data[, select_cols, drop = FALSE])[i]))
  }
}

# Mostrar el conteo de outliers
cat("Conteo de outliers:\n")
for (i in seq_along(outliers_counts)) {
  cat(names(data)[i], ": ", outliers_counts[[i]]$count, "\n")
}
...
print(summary(modelo_logit_lasso))
...
falsos_negativos_ridge <- confusion_matriz_ridge$table["1", "1"]
...
### Regresión logística estándar depurado
# Variables significativas
variables_significativas <- c(
  "FACULTADFACULTAD DE CIENCIAS AGRARIAS",
  "FACULTADFACULTAD DE CIENCIAS DE LA EDUCACION",
  "FACULTADFACULTAD DE CIENCIAS DE LA SALUD",
  "FACULTADFACULTAD DE CIENCIAS JURIDICAS Y SOCIALES",
  "FACULTADFACULTAD DE ECONOMIA Y NEGOCIOS",
  "FACULTADFACULTAD DE PSICOLOGIA",
  "GRUPO_ANHO_EGRESOULTIMO ANO",
  "UNIDAD_EDUCATIVA2",
  "UNIDAD_EDUCATIVA3",
  "DEPENDENCIASERVICIO LOCAL DE EDUCACION",
  "TIPO_EDUCACIONTECNICO PROFESIONAL",
  "PRIMERA_PREFRENCIA1",
  "PTJE_MATEMATICAS",
  "PTJE_NEM",
  "GRATUIDADSI",
  "TRABAJA_Y_ESTUDIASi",
  "SITUACION_DE_POSTULACIONSELECCIONADO",
  "CERCANIALEJOS",
  "EDAD_EGRESO_COLEGIO",
  "EDAD_INGRESO_UNIV"
)
...
X_train <- X_train[, colnames(X_train) %in% variables_significativas]
X_test <- X_test[, colnames(X_test) %in% variables_significativas]
data_train_significativas <- data.frame(X_train, DESERTA_1ER_ANHO = y_train)
data_test_significativas <- data.frame(X_test, DESERTA_1ER_ANHO = y_test)

# Ajustar el modelo logístico con las variables significativas
modelo_logit_significativo <- glm(DESERTA_1ER_ANHO ~ ., data = data_train_significativas)

summary(modelo_logit_significativo)
metricas_logitsignif <- calcualr_metricas_model(modelo_logit_significativo, data_test_significativas, y_test)

metricas_logitsignif
